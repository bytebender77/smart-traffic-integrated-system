...
VEH_COLORS = {
    ...
    "ambulance": [(60, 60, 245)],
    "firetruck": [(40, 120, 255), (30, 80, 220)],
}
...
detector_state = {
    ...
    "vehicle_types": {"car": 0, "truck": 0, "bike": 0, "ambulance": 0, "fire": 0},
    "emergency_type": None,
}
...
def _draw_hud(frame, sim_state, total_count, amb):
    ...
    if amb:
        ...
        label = "FIRE SERVICE" if em_type == "fire" else "AMBULANCE"
        cv2.putText(frame, f"{label} — MULTI-JUNCTION GREEN CORRIDOR ACTIVE",
...
def run_detector(sim_state_ref, sim_lock_ref):
    ...
    while True:
        ...
        with sim_lock_ref:
            ...
            em_type  = sim_state_ref.get("emergency_type")
        ...
        type_counts = {"car": 0, "truck": 0, "bike": 0, "ambulance": 0, "fire": 0}
        total_drawn = 0
        for v in veh_list:
            wx, wy = v["x"], v["y"]
            ...
            display_type = "fire" if vtype == "firetruck" else vtype
        ...
        _draw_hud(frame, {"sim_time": sim_time, "ambulance_detected": amb, "emergency_type": em_type}, total_drawn, amb)
        ...
        for jid, (jx, jy) in JUNCTION_PX.items():
            ...
            r2, j2 = cv2.imencode(".jpg", zoomed, [cv2.IMWRITE_JPEG_QUALITY, JPEG_QUALITY])
            if r2:
        ...
        if ret:
            with detector_lock:
                ...
                detector_state["emergency_type"] = sim_state_ref.get("emergency_type")
