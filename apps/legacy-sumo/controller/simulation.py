...
state = {
    ...
    "running": False,
    "mode": "AI",
    "ambulance_detected": False,  # legacy: true for any emergency vehicle
    "ambulance_id": None,         # legacy: id for any emergency vehicle
    "ambulance_route": [],
    "emergency_type": None,
    "emergency_id": None,
    "ambulance_events": 0,
    ...
    "alerts": [],
    "paused": False,
    # When set, the current SUMO run will exit early so the backend's
    # outer loop can immediately start a fresh simulation.
    "restart": False,
    "vehicles_pos": [],
    # Per-junction data
    "junctions": {
        jid: {
            "lanes": {
                "North": {"count": 0, "signal": "red", "green_time": 30, "wait_time": 0, "avg_speed_kmph": 0},
                "South": {"count": 0, "signal": "red", "green_time": 30, "wait_time": 0, "avg_speed_kmph": 0},
                "East":  {"count": 0, "signal": "red", "green_time": 30, "wait_time": 0, "avg_speed_kmph": 0},
                "West":  {"count": 0, "signal": "red", "green_time": 30, "wait_time": 0, "avg_speed_kmph": 0},
            },
...
# Speed-weighted control (prototype)
...
def _speed_weight(speed_kmph):
    ...
    return 1.0 + SPEED_WEIGHT * slowdown


def get_junction_speeds(jid):
    """Get average speed per approach for a specific junction (km/h)."""
    speeds = {"North": None, "South": None, "East": None, "West": None}
    for edge_id, approach in JUNCTION_APPROACHES[jid].items():
        lane_speeds = []
        for li in [0, 1]:
            lane_id = f"{edge_id}_{li}"
            try:
                s = traci.lane.getLastStepMeanSpeed(lane_id)
                # SUMO returns -1 when no vehicles on a lane
                if s is None or s < 0:
                    continue
        ...
        if lane_speeds:
            avg_mps = sum(lane_speeds) / len(lane_speeds)
            speeds[approach] = round(avg_mps * 3.6, 1)
        else:
            # Fallback to lane max speed or free-flow speed
            try:
                max_mps = traci.lane.getMaxSpeed(f"{edge_id}_0")
                speeds[approach] = round(max_mps * 3.6, 1)
            except:
                speeds[approach] = FREE_FLOW_SPEED_KMPH
    return speeds
...
def detect_ambulances():
    """Find all active emergency vehicles (ambulance + fire) and return their data."""
    emergencies = []
    try:
        for vid in traci.vehicle.getIDList():
            ...
            vtl = (vt or "").lower()
            vidl = vid.lower()
            if ("ambulance" in vidl or vtl == "ambulance" or
                    "fire" in vidl or vtl in ("fire", "firetruck")):
                ...
                vtype = "fire" if ("fire" in vidl or vtl in ("fire", "firetruck")) else "ambulance"
                emergencies.append({
                    "id": vid,
                    "type": vtype,
                    "route": route_edges,
...
def compute_ai_green_times(counts, speeds=None):
    """Density + speed-weighted green times for N+S vs E+W."""
    speeds = speeds or {}

    def eff(ap):
        count = counts.get(ap, 0)
        weight = _speed_weight(speeds.get(ap))
        return count * weight

    ns_eff = eff("North") + eff("South")
    ew_eff = eff("East") + eff("West")
    total = ns_eff + ew_eff or 1
    ns_ratio = ns_eff / total
    ew_ratio = ew_eff / total
...
def run_simulation(gui=False):
    ...
    try:
        while traci.simulation.getMinExpectedNumber() > 0 or step < 20:
            # Allow external callers to request an early restart.
            with state_lock:
            ...
            if amb_detected and not last_amb_detected:
                ambulance_events += 1
            last_amb_detected = amb_detected
            ...
            for jid in TL_IDS:
                ...
                if approaching_ambs:
                    ...
                    if sim_time - last_signal_eval >= SIGNAL_EVAL_INTERVAL:
                        ns_g, ew_g = compute_ai_green_times(counts, all_speeds.get(jid, {}))
                        ...
                        try:
                            ...
                            speeds = all_speeds.get(jid, {})
                            ns_count = (
                                counts.get("North", 0) * _speed_weight(speeds.get("North")) +
                                counts.get("South", 0) * _speed_weight(speeds.get("South"))
                            )
                            ew_count = (
                                counts.get("East", 0) * _speed_weight(speeds.get("East")) +
                                counts.get("West", 0) * _speed_weight(speeds.get("West"))
                            )
            ...
            agg_speed_weighted = {"North": 0.0, "South": 0.0, "East": 0.0, "West": 0.0}
            agg_speed_count = {"North": 0, "South": 0, "East": 0, "West": 0}
            for jid in TL_IDS:
                for ap in ["North", "South", "East", "West"]:
                    agg_counts[ap] += all_counts[jid].get(ap, 0)
                    agg_waits[ap] += all_waits[jid].get(ap, 0)
                    sp = (all_speeds.get(jid) or {}).get(ap)
                    cnt = all_counts[jid].get(ap, 0)
                    if sp is not None and cnt > 0:
                        agg_speed_weighted[ap] += sp * cnt
                        agg_speed_count[ap] += cnt

            # ── Alerts ──
            alerts = []
            for amb in active_ambulances:
                junc_str = " → ".join(amb["junctions"])
                icon = "🚒" if amb.get("type") == "fire" else "🚑"
                alerts.append({
                    "type": "emergency",
                    "message": f"{icon} {amb['id']} — Green corridor: {junc_str}",
                    "vehicle": amb["id"],
                })
            for jid in TL_IDS:
            ...
            with state_lock:
                ...
                state["ambulance_route"] = active_ambulances[0]["junctions"] if amb_detected else []
                state["emergency_type"] = active_ambulances[0]["type"] if amb_detected else ""
                state["emergency_id"] = active_ambulances[0]["id"] if amb_detected else ""
                state["ambulance_events"] = ambulance_events
                ...
                state["avg_wait_time"] = round(avg_wait, 2)
                state["time_saved_pct"] = time_saved
                state["alerts"] = alerts
                ...
                for jid in TL_IDS:
                    ...
                    for ap in ["North", "South", "East", "West"]:
                        ...
                        sp = (all_speeds.get(jid) or {}).get(ap)
                        state["junctions"][jid]["lanes"][ap]["avg_speed_kmph"] = sp if sp is not None else FREE_FLOW_SPEED_KMPH
                ...
                for ap in ["North", "South", "East", "West"]:
                    ...
                    if agg_speed_count[ap] > 0:
                        avg_sp = agg_speed_weighted[ap] / agg_speed_count[ap]
                    else:
                        avg_sp = FREE_FLOW_SPEED_KMPH
                    state["lanes"][ap]["avg_speed_kmph"] = round(avg_sp, 1)
