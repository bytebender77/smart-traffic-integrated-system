...
# Stream throttling (lower = smoother on weaker CPUs)
STREAM_FPS = 12
...
def get_traffic_data():
    ...
    with state_lock:
        return JSONResponse({
            ...
            "emergency_id":       state.get("emergency_id"),
        })
...
def get_emergency():
    ...
    with state_lock:
        ...
        return JSONResponse({
            ...
            "message": (
                f"{'🚒' if state.get('emergency_type') == 'fire' else '🚑'} Green corridor active for {state['ambulance_id']}"
...
def get_stats():
    ...
    with state_lock:
        amb_events = state.get("ambulance_events", 0)
        return JSONResponse({
            "sim_time":           state["sim_time"],
            "total_vehicles":     state["total_vehicles"],
            "vehicles_cleared":   state["vehicles_cleared"],
...
@app.post("/simulation/restart", tags=["Control"])
def restart_simulation():
    """
    Restart the SUMO run from T=0.

    Implementation detail: we set a `state["restart"]` flag that the
    simulation loop checks; it exits the current TraCI run early, and the
    backend's outer loop starts a fresh simulation.
    """
    with state_lock:
        state["paused"] = False
        state["restart"] = True
    with history_lock:
        history.clear()
    return JSONResponse({"status": "restarting", "message": "Simulation restart requested"})
